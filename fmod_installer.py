...
def list_versions(token: str):
    json = requests.get(
        fmod_downloads_link, headers={"Authorization": f"FMOD {token}"}
    ).json()
...
def copy_headers_and_libs(base_path: str, platform: str):
    ...
    shutil.copytree(
        core_lib, os.path.join(script_dir, "libs", platform), dirs_exist_ok=True
    )
    shutil.copytree(
        studio_lib, os.path.join(script_dir, "libs", platform), dirs_exist_ok=True
    )
...
def main():
    ...
    subparsers = parser.add_subparsers(
        dest="command", help="Descriptions", required=True, description=""
    )
    subparsers.add_parser(
        name="list_versions",
        help="List FMOD versions",
        description="List the fmod versions available for download",
    )
    dv = subparsers.add_parser(
        name="download_version",
        help="Download FMOD Version",
        description="Download the specified version from the web",
    )
    cs = subparsers.add_parser(
        name="install_cs", help="copy fmod C# wrappers to directory"
    )
    ...
    current_os = get_os()
    if args.targetplatform is None:
