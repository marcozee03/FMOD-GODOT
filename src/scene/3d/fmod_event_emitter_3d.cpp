...
void FmodEventEmitter3D::refresh_parameters() {
  ...
  int count;
  FMOD_Studio_EventDescription_GetParameterDescriptionCount(description,
                                                            &count);
  parameters.resize(count);
  for (int i = 0; i < count; i++) {
    FMOD_STUDIO_PARAMETER_DESCRIPTION param;
    FMOD_Studio_EventDescription_GetParameterDescriptionByIndex(description, i,
                                                                &param);
    parameters.write[i] = {param.name, cast_to_vector2i(param.id),
                           param.defaultvalue};
  }
}
void FmodEventEmitter3D::_bind_methods() {
  BIND_PROPERTY_WITH_HINT(event, Variant::VECTOR4I, PROPERTY_HINT_NONE,
                          "FmodEvent");
...
PackedStringArray FmodEventEmitter3D::_get_configuration_warnings() const {
  if (attached_to_rigidbody && !cast_to<RigidBody3D>(get_parent())) {
    return {"Parent must be a RigidBody3D when attached_to_rigidbody is true"};
  } else {
    return {};
  }
}
bool FmodEventEmitter3D::is_attached_to_rigidbody() const {
  return attached_to_rigidbody;
}
void FmodEventEmitter3D::set_attached_to_rigidbody(bool attached) {
  attached_to_rigidbody = attached;
  update_configuration_warnings();
}
bool FmodEventEmitter3D::_set(const StringName &p_name,
...
bool FmodEventEmitter3D::_get(const StringName &p_name,
...
bool FmodEventEmitter3D::_property_get_revert(const StringName &p_name,
                                              Variant &r_property) {
  if (!p_name.begins_with("param:")) {
    return false;
  }
  ...
  FMOD_Studio_EventDescription_GetParameterDescriptionByName(
      description, p_name.substr(6).utf8(), &param);
...
void FmodEventEmitter3D::_get_property_list(List<PropertyInfo> *p_list) const {
  ...
  int count;
  p_list->push_back(PropertyInfo(Variant::Type::STRING, "Parameters",
                                 PROPERTY_HINT_NONE, "",
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (param.flags & FMOD_STUDIO_PARAMETER_DISCRETE) {
      info.hint_string = String(",").join(
          {Variant(param.minimum), Variant(param.maximum), Variant(1)});
    } else {
      info.hint_string =
          String(",").join({Variant(param.minimum), Variant(param.maximum)});
    }
    p_list->push_back(info);
  }
}
...
void FmodEventEmitter3D::set_parameters() {
  if (FMOD_Studio_EventInstance_IsValid(event_instance)) {
    for (auto param : parameters) {
      FMOD_Studio_EventInstance_SetParameterByName(event_instance, param.name,
                                                   param.value, false);
    }
  }
}
...
bool FmodEventEmitter3D::validate_event_description() {
  if (!FMOD_Studio_EventDescription_IsValid(description)) {
    ...
    FMOD_RESULT result = FMOD_Studio_System_GetEventByID(
        FmodAudioServer::get_singleton()->get_studio(), &guid, &description);
...
void FmodEventEmitter3D::start() {
  ...
  if (!FMOD_Studio_EventInstance_IsValid(event_instance)) {
    ...
    if (is3D) {
      if (attached_to_rigidbody) {
        auto ptr = cast_to<RigidBody3D>(get_parent());
        if (ptr) {
          FmodAudioServer::get_singleton()->attach_instance_to_rigid_body3d(
              ptr, event_instance);
        } else {
          print_error("Event Emitter's parent is not a rigidbody3D");
          FmodAudioServer::get_singleton()->attach_instance_to_node3d(
              this, event_instance, this->non_rigid_body_velocity);
        }
      }
      FmodAudioServer::get_singleton()->attach_instance_to_node3d(
          this, event_instance, this->is_non_rigid_body_velocity());
    }
  }
  triggered = true;
  this->set_parameters();
  if (override_attenuation && is3D) {
    FMOD_Studio_EventInstance_SetProperty(
        event_instance, FMOD_STUDIO_EVENT_PROPERTY_MINIMUM_DISTANCE,
        attenuation_min);
    FMOD_Studio_EventInstance_SetProperty(
        event_instance, FMOD_STUDIO_EVENT_PROPERTY_MAXIMUM_DISTANCE,
        attenuation_min);
  }
...
void FmodEventEmitter3D::stop() {
  ...
  if (is_allow_fadeout()) {
    FMOD_Studio_EventInstance_Stop(event_instance,
...
Vector4i FmodEventEmitter3D::get_event() { return event_guid; }
...
bool FmodEventEmitter3D::is_one_shot() const { return one_shot; }
...
bool FmodEventEmitter3D::is_trigger_once() const { return trigger_once; }
...
void FmodEventEmitter3D::set_non_rigid_body_velocity(
...
bool FmodEventEmitter3D::is_allow_fadeout() const { return allow_fadeout; }
void FmodEventEmitter3D::set_allow_fadeout(bool p_allow_fadeout) {
  allow_fadeout = p_allow_fadeout;
}
void FmodEventEmitter3D::set_parameter(const String &name, float value) {
  for (int i = 0; i < parameters.size(); i++) {
...
void FmodEventEmitter3D::set_parameter_by_id(const Vector2i &id, float value) {
  for (int i = 0; i < parameters.size(); i++) {
...
float FmodEventEmitter3D::get_parameter_by_id(const Vector2i &p_id) const {
  for (int i = 0; i < parameters.size(); i++) {
