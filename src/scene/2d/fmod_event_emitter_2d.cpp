...
PackedStringArray FmodEventEmitter2D::_get_configuration_warnings() const {
  if (attached_to_rigidbody && !cast_to<RigidBody2D>(get_parent())) {
...
bool FmodEventEmitter2D::_set(const StringName &p_name,
...
bool FmodEventEmitter2D::_get(const StringName &p_name,
...
bool FmodEventEmitter2D::_property_get_revert(const StringName &p_name,
                                              Variant &r_property) {
  if (!p_name.begins_with("param:")) {
    return false;
  }
  ...
  if (!FMOD_Studio_EventInstance_IsValid(event_instance)) {
    ...
    if (is3D) {
      if (attached_to_rigidbody) {
        auto ptr = cast_to<RigidBody2D>(get_parent());
        if (ptr) {
          FmodAudioServer::get_singleton()->attach_instance_to_rigid_body2d(
              ptr, event_instance);
        } else {
          print_error("Event Emitters parent is not a rigidbody2D");
          FmodAudioServer::get_singleton()->attach_instance_to_node2D(
...
Vector4i FmodEventEmitter2D::get_event() { return event_guid; }
...
bool FmodEventEmitter2D::is_one_shot() const { return one_shot; }
...
bool FmodEventEmitter2D::is_trigger_once() const { return trigger_once; }
...
void FmodEventEmitter2D::set_non_rigid_body_velocity(
...
bool FmodEventEmitter2D::is_allow_fadeout() const { return allow_fadeout; }
...
bool FmodEventEmitter2D::is_attached_to_rigidbody() const {
  return attached_to_rigidbody;
}
void FmodEventEmitter2D::set_attached_to_rigidbody(bool p_attached) {
