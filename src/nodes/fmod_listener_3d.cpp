...
namespace FmodGodot
{
    void FmodListener3D::_physics_process(double delta)
    {
        if (is_attached_to_parent())
        {
            ...
            case RIGIDBODY3D:
            {
                FMOD_3D_ATTRIBUTES attributes = to_3d_attributes(rigid_parent3D);
                FMOD_Studio_System_SetListenerAttributes(FmodAudioServer::get_singleton()->get_studio(), listener_index, &attributes, NULL);
                return;
            }
            case NODE3D:
            {
                FMOD_3D_ATTRIBUTES attributes = to_3d_attributes(parent3D);
                FMOD_Studio_System_SetListenerAttributes(FmodAudioServer::get_singleton()->get_studio(), listener_index, &attributes, NULL);
                return;
            }
            default:
    ...
    void FmodListener3D::_notification(int p_what)
    {
        ...
        {
            RigidBody3D *rigidbody_parent = cast_to<RigidBody3D>(get_parent());
            Node3D *node3D_parent = cast_to<Node3D>(get_parent());
            if (rigidbody_parent)
            {
                _parent_type = RIGIDBODY3D;
                rigid_parent3D = rigidbody_parent;
            }
            else if (node3D_parent)
            {
                _parent_type = NODE3D;
                parent3D = node3D_parent;
            }
            else
            {
                _parent_type = NONE;
    ...
    int FmodListener3D::get_listener_index()
    {
        return listener_index;
    }
    void FmodListener3D::set_listener_index(int index)
    {
    ...
    bool FmodListener3D::is_attached_to_parent()
    {
        return attached_to_parent;
    }

    void FmodListener3D::set_attached_to_parent(bool p_attached_to_parent)
    {
